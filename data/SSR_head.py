...
class SSRHead(DETRHead):
    ...
    def _init_layers(self):
        ...
        #wm
        self.action_query = nn.Parameter(torch.randn(1, 1, self.embed_dims * 2))
        self.action_decoder = nn.MultiheadAttention(
            embed_dim=self.embed_dims * 2,
            num_heads=8,
            batch_first=True)
        self.action_mlp = nn.Sequential(
            nn.Linear(self.embed_dims * 2, 128),
            nn.ReLU(),
            nn.Linear(128, 9)
        )
        self.future_action_mlp = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(self.embed_dims, 9)  # nuscenes action
        )
    ...
    def forward(self,
        ...
        bev_navi_embed = self.navi_se(bev_embed, navi_embed)  #SE

        bev_query = torch.cat((bev_navi_embed, pos_embd), -1)   #带先验的query

        learned_latent_query, selected = self.tokenlearner(bev_query)  #tokenlearner
        # current_action_1
        action_query = self.action_query.expand(bs, -1, -1)
        action_feat, _ = self.action_decoder(
            action_query,
            bev_query,
            bev_query)

        current_action = self.action_mlp(action_feat)
        ...
        latent_query = self.latent_decoder(  #scene_queries
                query=latent_query,
                key=latent_query,
                value=latent_query,
        ...
        way_point = self.way_point.weight.to(dtype)  #本质query waypoint queries
        ...
        act_query = self.action_mln(latent_query, wp_vector)   #mln   scene_queries,traj
        # future action prediction
        future_action_feat = act_query.mean(0)  # (bs, embed_dim)
        future_action = self.future_action_mlp(future_action_feat)
